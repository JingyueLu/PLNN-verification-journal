...
def flatten_layers(layer_list, input_shape):
    new_layers = []
    for layer in layer_list:
        if type(layer) is nn.Linear:
            ...
            out_height = math.floor((in_height +2*layer.padding[0]- layer.dilation[0]*(layer.kernel_size[0]-1)-1)// layer.stride[0] + 1)
            out_width = math.floor((in_width +2*layer.padding[1] - layer.dilation[0]*(layer.kernel_size[1]-1)-1)// layer.stride[1] + 1)
            ...
            for in_c in range(in_chan):
                for in_h in range(in_height):
                    for in_w in range(in_width):
                        temp = layer.weight.new_full(input_shape, fill_value = 0.0).unsqueeze(0)
                        temp[0,in_c, in_h, in_w] = 1.0
                        out = (layer(temp).view(1, out_chan, -1) - conv_bias.unsqueeze(1)).view(-1)
                        eqlin_weight[:, in_w+in_h*in_width+in_width*in_height*in_c]= out
                        #pdb.set_trace()
