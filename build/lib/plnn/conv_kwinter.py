...
class KWConvNetwork(LinearizedNetwork):
    ...
    def get_lower_bound(self, relu_mask,  pre_lbs, pre_ubs, decision, choice, dual_info):
        try:
            start = time.time()
            gub, glb, ub_point, new_mask, lower_bounds, upper_bounds, dual_info= self.update_the_model(relu_mask, pre_lbs, pre_ubs, decision, choice,dual_info)
            end = time.time()
            print('KW_Int define_linear: ', end-start)
            return gub, glb,ub_point, new_mask, lower_bounds, upper_bounds, dual_info
    ...
    def build_the_model(self, input_domain, x, ball_eps):
        '''
        ...
        kw_lb, kw_ub, pre_relu_indices, dual_info = self.loose_dual.init_kw_bounds()
        ...
        for pre_idx in pre_relu_indices:
            if torch.sum(abs(lower_bounds[pre_idx]-kw_lb[pre_idx])>1e-4)==0 and torch.sum(abs(upper_bounds[pre_idx]-kw_ub[pre_idx])>1e-4)==0:
                ...
                lower_bounds, upper_bounds, dual_info = self.loose_dual.update_kw_bounds(dual_info, pre_idx, pre_lb_all = lower_bounds, pre_ub_all = upper_bounds)
                break

        ## DEBUG    
        #decision = [0,584]
        #choice = 1
        #init_lower, init_upper, init_dual = self.loose_dual.init_kw_bounds(lower_bounds, upper_bounds,decision, choice)
        #up_lower, up_upper, up_dual = self.loose_dual.update_kw_bounds(dual_info, pre_relu_indices[decision[0]], lower_bounds,upper_bounds, decision, choice, init_dual)
        #import pdb; pdb.set_trace()


        ## NEW STRUCTURE: use the computed bounds directly introduce gurobi models 
        ...
        for layer in self.layers:
            ...
            if type(layer) is nn.Linear:
                ...
                if isinstance(self.gurobi_vars[-1][0], list):
                    ...
                    new_layer_mask = [] 
                    bias_all = -pre_lbs*dual_info[0][layer_idx].I.squeeze(0).float()
        ...
        return gub, glb, mini_inp.unsqueeze(0), new_relu_mask, lower_bounds, upper_bounds, pre_relu_indices, dual_info


    def update_the_model(self, relu_mask, pre_lb_all, pre_ub_all, decision, choice, dual_info):
        '''
        ...
        lower_bounds, upper_bounds, new_dual_info  = self.loose_dual.update_kw_bounds(dual_info, first_changed_bounds_index, pre_lb_all, pre_ub_all, decision, choice)
        ...
        if change_idx < len(lower_bounds):
            print(f"update_kw interval is better: change_idx at {change_idx}")
            lower_bounds, upper_bounds, new_dual_info = self.loose_dual.update_kw_bounds(new_dual_info, change_idx, pre_lb_all = lower_bounds, pre_ub_all = upper_bounds)
        ...
        return gub, glb, mini_inp.unsqueeze(0), relu_mask, lower_bounds, upper_bounds, new_dual_info
